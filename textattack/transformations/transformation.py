...
class Transformation(ABC):
    ...
    def __call__(
        ...
        transformed_texts = self._get_transformations(current_text, indices_to_modify, original_text = original_text)
        for text in transformed_texts:
            text.attack_attrs["last_transformation"] = self
        return transformed_texts

    @abstractmethod
    def _get_transformations(self, current_text, indices_to_modify, original_text = None):
        """Returns a list of all possible transformations for ``current_text``,
