...
class WordSwapMaskedLM(WordSwap):
    ...
    def _bert_attack_replacement_words(
        ...
        if not len(target_ids_pos):
            ...
            over_thres_index = id_probs[target_ids_pos[0]] > threshold
            top_preds = id_preds[target_ids_pos[0]][over_thres_index].tolist()
            ...
            return replacement_words
        elif len(target_ids_pos) <= 4:
            # Word to replace is tokenized as multiple sub-words
            top_preds = [id_preds[i][:5] for i in target_ids_pos]
            ...
            top_replacements = [
                ...
                x[0] for x in combination_results[: self.max_candidates] if x[1] > threshold
            ]
            return top_replacements
        else:
            return []

    def _get_transformations(self, current_text, indices_to_modify, original_text):
        indices_to_modify = list(indices_to_modify)
        if self.method == "bert-attack":
            # print(current_text.text)
            # pause = input("??")
            current_inputs = self._encode_text(original_text.text)
            with torch.no_grad():
