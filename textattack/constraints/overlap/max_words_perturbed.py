...
class MaxWordsPerturbed(Constraint):
    ...
    def _check_constraint(self, transformed_text, reference_text):
        ...
        if self.max_percent:
            ...
            # max_words_perturbed = int(min_num_words * (self.max_percent))

            orig_text = reference_text.tokenizer_input
            # print(orig_text)
            if type(orig_text) == tuple or type(orig_text) == list:
                word_list = orig_text[1].split()
            else:
            ...
            # pause = input("???")
            # max_words_perturbed = math.ceil(min_num_words * (self.max_percent))
            max_words_perturbed = int(num_words * (self.max_percent))
